...
- Python 3.11+
- Ollama (local) or API keys for OpenAI, DeepSeek, or another OpenAI-compatible API
...
- **Multi-Provider Support**: Seamlessly switch between Ollama, OpenAI, DeepSeek, and other OpenAI-compatible endpoints
...
LLM_PROVIDER = "ollama"  # ollama | openai | deepseek | openai_compatible
...
These values can also be set in `project/.env`, which is loaded automatically by `project/app.py`.
...
> **Current implementation note:** The app now loads provider settings from `project/.env` and supports `ollama`, `openai`, `deepseek`, and `openai_compatible` without editing `project/core/rag_system.py`. If the examples below mention hand-editing the LLM import, prefer the `.env` approach instead.
